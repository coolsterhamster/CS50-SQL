/* You need a player that can get hits.
Who might be the most underrated? In 11.sql, write a SQL query to find the 10 least expensive players per hit in 2001.
...
SELECT p.first_name, p.last_name, (s.salary/pp.H) AS "dollars per hit"
FROM players p
JOIN salaries s ON p.id = s.player_id
JOIN performances pp ON s.player_id = pp.player_id AND s.year = pp.year
WHERE pp.H > 0
AND pp.year = 2001
